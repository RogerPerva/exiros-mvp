...
- **▶️ SIGUIENTE PASO (decisión de Rogelio):** opciones — **(A) Fase 5** (reportes: Excel 13 columnas + filtros) · **(B) H4 JWT** + W0 login (seguridad/auth, habilita admin real) · **(C) bloques WEB** alinear el portal al diseño (shell+sidebar+tabla+W3). ⚠️ KPI "<10% batería" e hibernación por ActivityRecognition NO verificables sin teléfono físico: nunca reportarlos cumplidos.
