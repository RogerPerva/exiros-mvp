...
- **▶️ SIGUIENTE PASO:** **FOCO = portal web fiel al diseño (Fase 10, ver PLAN).** Rogelio corrigió: el portal actual (vista única Mapa+tarjetas+Export) está MAL; el export es de **Viajes (W2)**, no de Mapa. Replicar EXACTO `docs/exiros-reference-image/webapp/`. Orden: **10.1 shell+sidebar+routing** → 10.2 Mapa → 10.3 Viajes (+mover export aquí) → 10.4 detalle (BE `GET /trips/:id`) → 10.5 Destinos (BE Fase 5.1) → 10.6 Usuarios (BE 6.2 con rol SUPER_ADMIN). **Decisiones:** 3 roles fieles (Super admin protegido); estados-mapa derivados POSPUESTOS (solo En ruta/Concluido). HECHO esta sesión (pend. confirmar): 6.1 JWT · 7.1 export · 7.2 export web (a reubicar en Viajes) · 6.3 parcial (login+sesión). ⚠️ batería/hibernación NO verificables sin teléfono. ⚠️ H4 cerrado; **H5 = Fase 10**.
