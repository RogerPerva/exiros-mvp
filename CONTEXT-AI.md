...
- **HUECOS RESTANTES (priorizados):** **H4** `/api/web/*` sin Guard JWT (cualquiera lista viajes) → Fase 6.1. **Fase 4** cierre por geocerca (haversine sobre los 2 puntos frescos/precisos — la selección se difirió aquí; el backend responde `stopTracking:false` placeholder y el `TripTokenGuard` debe pasar a **200 + stopTracking:true** para viaje CONCLUIDO en vez del 401 actual). **H5** la web diverge del diseño: falta shell+sidebar (Mapa·Viajes·Destinos·Usuarios), W2 debe ser **tabla** (hoy tarjetas), W1 con KPIs/clusters → bloques WEB ~6.x. **H6** `/health` → Fase 9. ~~D-A~~ **cerrado** (openapi declarado normativo; respuesta 3.4 alineada al spec). Menores: constant-time en `AppKeyGuard`; ingesta no exactamente-once si llega un punto nuevo entre reintentos (batchId cambia) — aceptable para MVP.
