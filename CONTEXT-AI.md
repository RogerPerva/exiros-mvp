...
- **▶️ SIGUIENTE PASO:** orden acordado con Rogelio (cimientos→fachada): **6.1 JWT [HECHO 2026-06-22, pend. confirmar]** → **(siguiente) Fase 7 reportes** (7.1 export Excel 13 columnas + 7.2 filtros; nacen protegidos por el Guard JWT) → **Fase 5 destinos CRUD** + **6.2/6.3** (usuarios + W0 login web + W5) → con eso, **bloques WEB (C)** alinear el portal al diseño (shell+sidebar+tabla+W3) con contenido real en cada sección. ⚠️ KPI "<10% batería" e hibernación por ActivityRecognition NO verificables sin teléfono físico: nunca reportarlos cumplidos. ⚠️ **H4 cerrado por 6.1**: `/api/web/*` ya exige JWT y el cierre admin registra `closedById`.
...
- **2026-06-22 (sesión Fase 3+4, la de hoy):** Fase 3 completa (rastreo por lotes GZIP + cierre auto por geocerca) + Fase 4 completa (cierres operador/admin, M4/M5, stop GPS, forzar cierre web) + alineación al spec + Fase 1 y 2 confirmadas por Rogelio. → checkpoint vigente arriba.
