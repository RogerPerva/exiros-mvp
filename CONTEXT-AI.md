...
- **HUECOS CERRADOS:** **H4** (`/api/web/*` con Guard JWT; cierre admin registra `closedById`) y **H5** (web alineada al diseño, Fase 10) — RESUELTOS. **RESTANTES (no bloquean; ninguno es un bloque del plan):** **H6** `/health` — DIFERIDO, NO se hizo en Fase 9 (la demo no lo necesita; útil para auditor/deploy). Diferidos por decisión de Rogelio: estados-mapa derivados (Detenido/Sin actualización/Cerca de destino) + capa Clusters. Menores conocidos: constant-time en `AppKeyGuard`; ingesta no exactamente-once entre reintentos; cierres admin/operador con `window.prompt` (MVP); `fallbackToDestructiveMigration` en Room; export de Viajes sin filtro Destino (listado trae nombre, no id).
- **▶️ SIGUIENTE PASO:** **No quedan bloques pendientes** — Fases 1–10 marcadas `[x] (pendiente confirmar)`, todo pusheado. Lo que falta es **validación humana de Rogelio** (recorrer portal + ver demo) para poner `✔ Confirmado`. Opcional/post-MVP: decisiones diferidas (estados-mapa derivados, capa Clusters), deploy cloud real (D2 con Julio, hoy solo túnel), endpoint `/health` (H6). KPI batería no medible sin teléfono.
- **⚠️ PARA ROGELIO (lo que TÚ debes resolver/validar — el agente no puede):**
  ...
  4. **Datos de prueba en la BD dev** acumulados al verificar: varios viajes CONCLUIDO de "Transporte del Norte SA de CV" (folio 100294, placa ABC-12-34), destinos "Patio Comprador Monterrey"/"Patio Vendedor Saltillo", usuario "María Pérez" — bórralos si molestan para la demo.
