...
**Implementadas:** #2 tripToken Guard (anti-IDOR: `:id`==viaje del token), #3 rate-limit por tripToken en la ingesta (`IngestThrottlerGuard`, ~60/min por token, configurable), #4 validación estricta, #5 tope body 256kb, #6 exception filter, #8 token hasheado (sha256), #9 idempotencia `batchId`/`closeRequestId`, #11 helmet+CORS, #12 rate-limit por IP real. **Extra:** `AppKeyGuard` en **tiempo constante** (`timingSafeEqual`); **foto** con extensión derivada del MIME validado **y verificación del tipo real por _magic number_** (bytes de cabecera, no el `Content-Type` del cliente). **Web:** el Guard JWT, además de verificar firma/expiración, **revalida contra BD que el usuario siga existiendo y activo** en cada request → un usuario dado de baja o degradado deja de operar de inmediato (no espera a que expire el token). **Pendiente (post-MVP):** #10 chequeo de teletransporte (anotado, no construido).
