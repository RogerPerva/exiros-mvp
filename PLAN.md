...
> **🟢 ESTADO (2026-06-26): TODAS las Fases 1–10 completas; MVP funcional e2e; EC2 vivo; APK release compilado.** Añadido tras la demo-prep: validación estricta de campos (Núm. Proveedor/Folio numéricos), primer punto inmediato al iniciar viaje, **modo demo de rastreo** (`-PEXIROS_DEMO_SECONDS`), y pulido de UX de mapa (fitBounds a la ruta, zoom al seleccionar viaje, alto del mapa de detalle). Pendiente: validación humana (`✔ Confirmado`), **cerrar el BLOCKER operativo del EC2** (supervisor del backend/túnel), y KPI batería (no medible sin teléfono). El estado vivo y el "cómo levantar" están en [`CONTEXT-AI.md`](CONTEXT-AI.md) (checkpoint vigente). Lo de abajo (decisiones, fases, bitácora) es el **registro histórico** del plan: refleja su historia, no tareas abiertas.
