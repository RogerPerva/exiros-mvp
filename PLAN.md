...
> **🟢 ESTADO (2026-06-26): TODAS las Fases 1–10 completas; MVP funcional e2e; EC2 vivo; APK release compilado.** Añadido tras la demo-prep: validación estricta de campos (Núm. Proveedor/Folio numéricos), primer punto inmediato al iniciar viaje, **modo demo de rastreo** (`-PEXIROS_DEMO_SECONDS`), y pulido de UX de mapa (fitBounds a la ruta, zoom al seleccionar viaje, alto del mapa de detalle). Pendiente: validación humana (`✔ Confirmado`), **cerrar el BLOCKER operativo del EC2** (supervisor del backend/túnel, `QA.md` QA-B01), y KPI batería (no medible sin teléfono). El estado vivo y el "cómo levantar" están en [`CONTEXT-AI.md`](CONTEXT-AI.md) (checkpoint vigente); los riesgos/pendientes vivos en [`QA.md`](QA.md). Lo de abajo (decisiones, fases, bitácora) es **rastro de auditoría**: refleja el plan y su historia, no tareas abiertas.
...
- **2026-06-26 (QA: validación de campos + punto de cierre + mapa de concluidos + filtros):** Sesión de QA pedida por Rogelio. **(1) Validación estricta de caracteres (doc §3.2)** en las 3 capas: Android filtra en la fuente Nombre de Proveedor (alfanumérico + `. , & -` para razón social) y placas (`[A-Z0-9-]` mayúsc.); web con saneadores `web/src/validation.ts` (nombre destino/usuario + formato email); backend `@Matches` en `CreateTripDto` (providerName y placas) como defensa en profundidad + tests negativos e2e. Decisión Rogelio: nombres permiten puntuación básica (`. , & -`), no estrictos. **(2) "Punto de cierre":** nuevas columnas `Trip.endLat/endLng` (migración `2026_06_25_004`) que fijan la última coordenada guardada al cerrar — manual operador/admin (`TripCloseService`) **y** automático por geocerca (`LocationsService`, último punto elegible). No tocó Android: el SyncWorker ya sube los puntos antes de cerrar. **(3) Mapa muestra concluidos:** `TripsMap`/`MapaPage` ya no excluyen los CONCLUIDO; se pintan en gris en su punto final (`lastLocation`) y aparecen en la tabla. Antes el filtro "Concluido" del mapa dejaba todo vacío. **(4) Retoques UI:** botón "Limpiar filtros" en Usuarios; búsqueda+estado+limpiar en Destinos (no tenía filtros); paginación (8/pág) en la tabla del Mapa. **Diagnóstico clave (NO era bug):** "Concluidos vacío" en la webapp era la **pantalla Mapa** (excluía concluidos a propósito) **contra el backend de EC2**; confirmado con A/B local (BD local tenía 22 concluidos) → era del sistema, ya corregido. **Gotcha de deploy:** la migración `endLat/endLng` falta aplicar en el RDS de AWS (`prisma migrate deploy`) o el backend nuevo no arranca. Gates verdes 3 capas; verificado en navegador (Playwright) en local. Commits: `be03a86` + cierre de sesión.
